...
It does not claim that judge repeatability, SATURATION-01, leaf construction, or acceptance experiments have passed. The provider-backed Day 1 Gate and local BGE-M3 sub-Gate have passed; later qualification Gates remain pending.
...
- Every generated data/power artifact records `full_leaf_generation_allowed=false`. Full leaf generation remains forbidden because protocol freeze, SATURATION, evaluator qualification, and later Gates are not complete; a passed Day 1 Gate does not authorize leaves.
