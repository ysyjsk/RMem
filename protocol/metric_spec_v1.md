...
`AcceptedOutputBindingRaw` is the only truth for an accepted output. A
successful binding references exactly one accepted attempt, and an attempt can
be referenced by at most one successful binding. The optional
`ModelCallAttemptRaw.accepted_attempt` field is only a consistency cache.
Bindings cover leaf, merge, answer, and judge stages.

Every API stage uses the same `ModelCallAttemptRaw` fields. Local token fields
are used for capacity and reproducibility; provider fields are operational
observations and are never populated from local counts. `provider_usage_source`
is `provider_exact`, `provider_estimated`, or `missing`. When it is `missing`,
all provider token fields are null and the corresponding provider resource
metric is `unknown`.
...
`AcceptedOutputBindingRaw` remains the only accepted-output truth. Raw transport,
model-attempt, binding, and output artifacts are kept separate from the derived
observation table and metrics.
...
For a generated merge with child memory lengths `L` and `R` and output budget
`B_merge`:

```text
content_to_budget_pressure = (L + R) / B_merge
actual_compression_ratio   = output_memory_tokens / (L + R)
output_budget_utilization  = output_memory_tokens / B_merge
payload_to_budget_ratio    = local_serialized_input_tokens / B_merge
...
semantic retention. `generative_rewrite_depth` counts only generative merge
operators on the semantic path and is unaffected by retries, failed attempts,
or cache hits. Earlier/later roles are determined by frozen evidence order and
covered spans, never by timestamps.
